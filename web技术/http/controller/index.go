...
func loginPostTask(c *gin.Context) {
	...
	if !online { //redis中没有该用户
		...
		*/
		var name, pwd string
		sqlQuery := `select  "uname","upwd" from public.usert where uname=$1`
	...
	row := PgDB.QueryRow(sqlQuery, form.User)
	err := row.Scan(&name, &pwd)
	fmt.Println("name = "+form.User+"\tpassword = ", form.Password)
	fmt.Println("sql name = "+name+"\tpassword = ", pwd)
...
func uploadFiles(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"err": "打开文件夹错误"})
		return
	}
	...
	for _, file := range files {
		...
		fileName := dstPath + strconv.Itoa(dirLen) + ".jpg"
		dirLen++
		//写入文件
		if err := c.SaveUploadedFile(file, fileName); err != nil {

			log.Println(err)
			c.JSON(http.StatusBadRequest, gin.H{"err": "上传文件错误"})
